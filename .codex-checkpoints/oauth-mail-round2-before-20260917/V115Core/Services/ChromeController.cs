...
public sealed partial class ChromeController : IAsyncDisposable
{
    const string TikTokIdentityDiagnosticBinaryMarker =
        ...
        "SAVE_NOT_COMMITTED|POST_SAVE_VERIFY_FAILED_AFTER_4_REFRESHES|NAME_GUARD_NAME_RETRY_BLOCKED";
...
"""));
        ...
        // Transaction Save/Confirm đã kết thúc đúng một lần. Bốn round bên dưới chỉ
        ...
        for (var round = 1; round <= 4 && !allVerified; round++)
        {
            ct.ThrowIfCancellationRequested();
            _log.Info($"[TIKTOK_IDENTITY_POST_SAVE_WAIT] round={round}/4 seconds=15");
            ...
            {
                _log.Warn($"[TIKTOK_IDENTITY_PROFILE_REFRESH_WARN] round={round}/4 message={ex.Message}");
            }
            _log.Info($"[TIKTOK_IDENTITY_PROFILE_REFRESH] round={round}/4 pageReady={pageReady.ToString().ToLowerInvariant()}");
            ...
            if (pageReady)
            {
                ...
                while (DateTime.UtcNow < roundReadDeadline)
                {
                    ...
                    {
                        _log.Warn($"[TIKTOK_IDENTITY_VERIFY_PROFILE_WARN] round={round}/4 message={ex.Message}");
                    }
            ...
            _log.Info(
                $"[TIKTOK_IDENTITY_VERIFY_PROFILE] round={round}/4 " +
            ...
            if (allVerified)
            {
                ...
                _log.Info($"[TIKTOK_IDENTITY_COMMIT_CONFIRMED] round={round}/4 status=SUCCESS");
                break;
            }

            if (round < 4)
                _log.Info($"[TIKTOK_IDENTITY_COMMIT_PENDING] round={round}/4");
        }
        ...
        var finalReason = allVerified
            ? "COMMIT_CONFIRMED"
            : "POST_SAVE_VERIFY_FAILED_AFTER_4_REFRESHES";
        _log.Info(
...
""";
        ...
        CancellationToken ct)
    {
        ...
        DateTimeOffset? emailOtpSubmittedAt = null;
        ...
        while (DateTime.UtcNow < loginDeadline)
        {
            ...
            if (await DetectEmailVerificationAsync(ct))
            {
                ...
                if (emailOtpSubmittedAt is not null
                    && DateTimeOffset.UtcNow - emailOtpSubmittedAt < TimeSpan.FromSeconds(12))
                {
                ...
                var triggerState = await TriggerEmailVerificationCodeAsync(ct);
                ...
                emailOtpSubmittedAt = DateTimeOffset.UtcNow;
...
(() => {
  ...
  const norm=x=>String(x||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/đ/g,'d').replace(/\s+/g,' ').trim();
  const inputs=[...document.querySelectorAll('input')].filter(visible);
  const hasCodeInput=inputs.some(e=>Number(e.maxLength||0)===6 || e.inputMode==='numeric' || /code|otp|verify/.test(norm((e.name||'')+' '+(e.placeholder||''))));
...
""", ct: ct);
        if (!fillResult.TryGetProperty("value", out var filled)
            || filled.ValueKind != JsonValueKind.True)
        {
            return "OTP_INVALID";
        }
