...
public sealed class SettingsService
{
    ...
    public AppSettings Load()
    {
        ...
        s.MailOtp = new MailOtpSettings
        {
            TimeoutSeconds = Math.Clamp(ini.GetInt("MailOtp", "TimeoutSeconds", 60), 10, 300),
            PollIntervalSeconds = Math.Clamp(ini.GetInt("MailOtp", "PollIntervalSeconds", 2), 1, 30),
            MaxRetry = Math.Clamp(ini.GetInt("MailOtp", "MaxRetry", 2), 1, 5)
        };
    ...
    public void Save(AppSettings s)
    {
        ...
        ini.Set("MailOtp", "TimeoutSeconds", Math.Clamp(s.MailOtp.TimeoutSeconds, 10, 300));
        ini.Set("MailOtp", "PollIntervalSeconds", Math.Clamp(s.MailOtp.PollIntervalSeconds, 1, 30));
        ini.Set("MailOtp", "MaxRetry", Math.Clamp(s.MailOtp.MaxRetry, 1, 5));
