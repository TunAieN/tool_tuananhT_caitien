...
public sealed record TikTokAuthMaterial(
    string Username,
    string Password,
    string TotpSecret,
    bool AutoLogin,
...
public sealed class TikTokAuthService
{
        ...
        bool autoLogin = true)
    {
        ...
        var settings = new TikTokAuthSettings
        {
            ...
            PasswordProtected = Protect(account.Password ?? ""),
            TotpSecretProtected = Protect(totpSecret),
            AutoLogin = autoLogin,
            LoginMode = account.LoginMode.ToString(),
            Email = (account.Email ?? "").Trim(),
            EmailPasswordProtected = Protect(account.EmailPassword ?? ""),
            RefreshTokenProtected = Protect(account.RefreshToken ?? ""),
            ClientId = (account.ClientId ?? "").Trim(),
            MailKpi = (account.MailKpi ?? "").Trim()
        };
    ...
    public TikTokAuthMaterial Load(string dataRoot)
    {
        ...
        try
        {
            ...
            var loginMode = Enum.TryParse<TikTokLoginMode>(settings.LoginMode, true, out var parsedMode)
                ? parsedMode
                : TikTokLoginMode.Normal;
            return new TikTokAuthMaterial(
                ...
                loginMode,
                (settings.Email ?? "").Trim(),
                Unprotect(settings.EmailPasswordProtected),
                Unprotect(settings.RefreshTokenProtected),
                (settings.ClientId ?? "").Trim(),
                (settings.MailKpi ?? "").Trim());
        }
        catch (Exception ex)
        {
