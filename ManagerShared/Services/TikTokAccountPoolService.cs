...
public sealed record TikTokAccountPoolItem(
    ...
    int SourceRow,
    TikTokLoginMode LoginMode = TikTokLoginMode.Normal,
    string Email = "",
...
public sealed class TikTokAccountPoolService
{
    ...
    public TikTokAccountImportResult ImportExcel(string path)
    {
        ...
        if (TryReadRawAccountList(path, out var rawAccounts, out var invalidFormat, out var rawTotalRows))
        {
            var existingRaw = Load();
            var existingByUsername = existingRaw
                .GroupBy(x => x.Username, StringComparer.OrdinalIgnoreCase)
                .ToDictionary(g => g.Key, g => g.First(), StringComparer.OrdinalIgnoreCase);
            var sameRawSource = SamePath(oldSourcePath, path);

            var imported = rawAccounts.Select(entry =>
            {
                existingByUsername.TryGetValue(entry.Account.Username, out var old);
                return new TikTokAccountPoolItem(
                    old?.Id ?? Guid.NewGuid().ToString("N"),
                    entry.Account.Username,
                    entry.Account.Password,
                    "",
                    old?.Note ?? "",
                    sameRawSource ? old?.AssignedProfile ?? "" : "",
                    entry.SourceRow,
                    entry.Account.LoginMode,
                    entry.Account.Email,
                    entry.Account.EmailPassword,
                    entry.Account.RefreshToken,
                    entry.Account.ClientId,
                    entry.Account.MailKpi);
            }).ToList();

            SaveCatalog(imported, path);
    ...
    public void Upsert(TikTokAccountPoolItem item)
    {
        ...
        var rawAccountList = !string.IsNullOrWhiteSpace(sourcePath)
                             && File.Exists(sourcePath)
                             && TryReadRawAccountList(sourcePath, out _, out _, out _);
        ...
        var normalized = item with
        {
            ...
            SourceRow = item.SourceRow <= 0 || (item.SourceRow == 1 && !rawAccountList)
                ? NextSourceRow(items)
                : item.SourceRow
        };
        ...
        StoredCatalog stored)
    {
        return stored.Accounts
            ...
            .Select(x => new TikTokAccountPoolItem(
                ...
                Enum.TryParse<TikTokLoginMode>(x.LoginMode, true, out var loginMode)
                    ? loginMode
                    : TikTokLoginMode.Normal,

                (x.Email ?? "").Trim(),
                Unprotect(x.EmailPasswordProtected),
                Unprotect(x.RefreshTokenProtected),
                (x.ClientId ?? "").Trim(),
                (x.MailKpi ?? "").Trim()))
            .ToList();
    }
        ...
        var catalog = new StoredCatalog
        {
            ...
            Accounts = items
                ...
                .Select(x => new StoredItem
                {
                    ...
                    LoginMode = x.LoginMode.ToString(),
                    Email = (x.Email ?? "").Trim(),
                    EmailPasswordProtected = Protect(x.EmailPassword ?? ""),
                    RefreshTokenProtected = Protect(x.RefreshToken ?? ""),
                    ClientId = (x.ClientId ?? "").Trim(),
                    MailKpi = (x.MailKpi ?? "").Trim(),
    ...
    static bool TryReadRawAccountList(
        string path,
        out List<(TikTokLoginAccount Account, int SourceRow)> accounts,
        out int invalidFormat,
        out int totalRows)
    {
        accounts = [];
        invalidFormat = 0;
        ...
        var firstContent = lines.FirstOrDefault(x => !string.IsNullOrWhiteSpace(x));
        if (TikTokAccountParser.IsHeaderLine(firstContent)
            || !lines.Any(TikTokAccountParser.LooksLikeAccountLine))
            return false;
        ...
        for (var index = 0; index < lines.Length; index++)
        {
            ...
            totalRows++;
            var parsed = TikTokAccountParser.Parse(lines[index]);
            if (!parsed.Success || parsed.Account is null)
            {
                invalidFormat++;
                continue;
            }

            accounts.Add((parsed.Account, index + 1));
        }
        ...
        TikTokAccountPoolItem item)
    {
        ...
        if (TryReadRawAccountList(path, out _, out _, out _))
        {
            var lines = File.ReadAllLines(path, Encoding.UTF8).ToList();
            while (lines.Count < item.SourceRow)
                lines.Add("");
            lines[item.SourceRow - 1] = SerializeRawAccount(item);
    ...
    static string SerializeRawAccount(TikTokAccountPoolItem item)
        => item.LoginMode == TikTokLoginMode.OAuthMail
            ? string.Join('|',
                item.Username,
                item.Password,
                item.Email,
                ...
                item.ClientId,
                item.MailKpi)
            : string.Join('|', item.Username, item.Password);
        ...
        SourceColumnLayout columns)
    {
             ...
             rowIndex++)
        {
            ...
            var email =
                GetCell(row, columns.Email)
                    .Trim();

            var emailPassword =
                GetCell(row, columns.EmailPassword)
                    .Trim();

            var refreshToken =
                GetCell(row, columns.RefreshToken)
                    .Trim();

            var clientId =
                GetCell(row, columns.ClientId)
                    .Trim();

            var mailKpi =
                GetCell(row, columns.MailKpi)
                    .Trim();

            // Keep the XLSX decision aligned with the runtime contract instead of
            // treating every row containing an email address as OAuth Mail.
            var oauthCandidate = new TikTokLoginAccount(
            ...
            var loginMode = oauthCandidate.HasOAuthMailbox
                ? TikTokLoginMode.OAuthMail
                : TikTokLoginMode.Normal;
        ...
        bool allocateManagedColumns)
    {
        ...
        var email = FindHeader(
            headers,
            "email");

        var emailPassword = FindHeader(
            headers,
            "mật khẩu email",
            "mat khau email",
            "email password",
        ...
        var refreshToken = FindHeader(
            headers,
            "refresh token",
            "refreshtoken");

        var clientId = FindHeader(
            headers,
            "client id",
            "clientid");

        var mailKpi = FindHeader(
            headers,
            "mail kpi",
            "mailkpi",
            "kpi mail");
