...
public sealed partial class ManagerForm
{
    ...
    (ProfileContext Context, bool CreatedNow) EnsureAutoProfileExists(AutoProfileQueueItem item)
    {
        ...
        if (existing is not null)
        {
            ...
            _tiktokAuthService.Save(dataRoot, ToLoginAccount(item.Account), item.Account.TotpSecret, autoLogin: true);
    ...
    static TikTokLoginAccount ToLoginAccount(TikTokAccountPoolItem account)
        => new(
            account.LoginMode,
            account.Username,
            account.Password,
    ...
    async Task EnsureAutoProfileLoggedInAsync(ProfileContext ctx, AutoProfileQueueItem item, CancellationToken ct)
    {
        ...
        for (var attempt = 1; attempt <= 2; attempt++)
        {
            ...
            last = await SendCommandAsync(ctx, "launch_auto", TimeSpan.FromSeconds(360));
            ...
            if (string.Equals(last, "need_manual", StringComparison.OrdinalIgnoreCase))
                throw new AutoProfilePauseException("PAUSED_LOGIN_MANUAL", "WAIT_LOGIN", "TikTok yêu cầu bước xác minh thủ công; tool không tự xử lý challenge.");
            if (string.Equals(last, "mail_token_invalid", StringComparison.OrdinalIgnoreCase))
                throw new AutoProfilePauseException("MAIL_TOKEN_INVALID", "WAIT_LOGIN", "Refresh token Outlook không hợp lệ; không retry vô hạn.");
            if (string.Equals(last, "mail_permission_error", StringComparison.OrdinalIgnoreCase))
                throw new AutoProfilePauseException("MAIL_PERMISSION_ERROR", "WAIT_LOGIN", "Microsoft Graph từ chối quyền đọc Inbox; không retry vô hạn.");
            if (string.Equals(last, "otp_invalid", StringComparison.OrdinalIgnoreCase)
                || string.Equals(last, "otp_submit_failed", StringComparison.OrdinalIgnoreCase))
                throw new AutoProfilePauseException(last.ToUpperInvariant(), "WAIT_LOGIN", "OTP email không hợp lệ hoặc không được TikTok chấp nhận.");
